...
def test_28_factors_have_aliases():
    """US-001 AC1: 29 因子全有 aliases，每个至少 1 个业界通用名（D-013.1：27+T6/T7）"""
    assert len(FACTOR_REGISTRY) == 29, f"Expected 29 factors, got {len(FACTOR_REGISTRY)}"
    assert len(ALIASES_REGISTRY) == 29, f"Expected 29 alias entries, got {len(ALIASES_REGISTRY)}"
