...
因子清单（US-002 删 M6 后剩 21 个继承；27 仓 = 21 inherited + 1 W4_rv + 1 T5_ma5 + 4 独立文件 B1/T1/V1/W4）：
    T2 MA 多头, T3 SAR 趋势, T4 ADX 趋势
    M1 动量 3 日, M2 动量 5 日, M3 动量 10 日, M4 RSI, M5 KDJ
    V2 OBV, V3 MAOBV, V4 量比
    W1 ATR, W2 布林带宽, W3 波动率
    S1 VHF, S2 ADX 强度（US-002 改名为 S2_adx_strength）
    O1 CCI, O2 WR
    R1 相对强弱
    N1 3 日反转, N2 5 日反转, N3 RSI 超卖反弹
    （M6 MACD 差值已删，公式与 T1_macd_bar 完全重复）

被谁调用（按规则 18 / v1 L118）：
    - src/etf_quant/alpha/factors/__init__.py（注册 21 因子到 FACTOR_REGISTRY）
    - src/etf_quant/alpha/registry.py（向后兼容导出）
    - src/etf_quant/alpha/analysis/batch_ic.py（IC 批量计算）
    - src/etf_quant/backtest/comprehensive_validator.py（4 验证器跑单因子）
    - skills/quantor-onboard/scripts/run_alpha.py（散户 alpha 引导）
    - tests/unit/alpha/test_factors.py（21 因子单测）
